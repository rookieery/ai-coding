...
| 房间业务逻辑（含孤儿房间启动清理 + 原子观战计数） | `gomoku-server/src/services/room.service.ts` |
...
| 对弈逻辑单元测试（35 个测试用例） | `gomoku-server/src/__tests__/online-game.test.ts` |
...
| 房间事件处理（含观战验证 + room:unwatch 取消观战 + 大厅状态同步广播） | `gomoku-server/src/socket/handlers/room.handler.ts` |
