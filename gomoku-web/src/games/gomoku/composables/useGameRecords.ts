...
export function useGameRecords(gameType: GameType) {
  ...
  const saveGame = async (
    ...
    try {
      const savedGame = await gameApi.saveGame(game);
      await fetchRecords();
      return savedGame;
    } catch {
      return null;
  ...
  const updateGame = async (id: string, game: FrontendGame): Promise<boolean> => {
    ...
    try {
      await gameApi.updateGame(id, game);
      await fetchRecords();
      return true;
    } catch {
      return false;
  ...
  const saveEdit = async (notify: (msg: string) => void): Promise<boolean> => {
    ...
    if (index !== -1) {
      ...
      try {
        ...
        const updatedGame: FrontendGame = { ...fullGame as FrontendGame, name };
  ...
  const getFullGame = async (id: string): Promise<FrontendGame | null> => {
    try {
      const game = await gameApi.getGame(id, gameType);
      return game as FrontendGame;
    } catch {
      return null;
    }
  };
