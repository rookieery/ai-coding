...
export function useGameUI() {
  ...
  const getStatusText = (
    ...
    if (winner === BLACK) return t('statusBlackWin');
    if (winner === WHITE) return t('statusWhiteWin');
    if (winner === 3) return t('statusDraw');

    const turnText = currentPlayer === BLACK ? t('statusBlackTurn') : t('statusWhiteTurn');
  ...
  const getAiRoleText = (aiRole: 'first' | 'second'): string => {
    return aiRole === 'first' ? t('black') : t('white');
  };
