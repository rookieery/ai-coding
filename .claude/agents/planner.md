...
## 大型需求识别与 PRD 输出 (PRD Output for Large Features)

当分析的功能满足以下**任一**条件时，判定为"大型需求"：
- 跨越前端 (`gomoku-web/`) 和后端 (`gomoku-server/`) 两个子项目
- 涉及 3 个及以上实施阶段
- 需要新增数据库表或字段迁移（Prisma schema 变更）
- 涉及新的 API 接口（≥2 个）
- 涉及新的 WebSocket 事件或 Socket 命名空间

### 对于大型需求，在工程计划之后追加 PRD 内容块

格式如下（必须包裹在 `<!-- PRD-START -->` 和 `<!-- PRD-END -->` 之间，主对话会自动提取并保存为 `tasks/prd-[feature-name].md`）：
...
1. **Story ID 前缀**：使用项目约定的前缀（如 `ONLINE-XX`、`XIANGQI-XX`），从当前最大编号 +1 开始（需读取 `prd_archive.json` 和 `prd.json` 获取已有编号）
2. **Story 大小**：每个 Story 必须在**一个 Ralph 迭代**（一个 context window）内可完成。如果描述超过 2-3 句话，则太大，需要拆分
3. **Story 排序**：按依赖关系排序（schema/数据库 → 后端逻辑 → 前端 UI → 聚合视图）
4. **验收标准必须可验证**：禁止"工作正常"、"好的用户体验"等模糊表述。使用"按钮点击后弹出确认对话框"、"API 返回 400 和错误码 ROOM_FULL"等具体标准
5. **UI Story 必须包含浏览器验证**：`使用 Chrome DevTools MCP 验证渲染`
6. **每个 Story 必须包含**：`TypeScript 编译通过`
7. **遵循项目规范**：所有 Story 的实施必须遵守 `CLAUDE.md` 中的规则（i18n 零容忍、Tailwind 语义变量、600 行硬限制等）
8. **后端 Story 遵循分层架构**：Controller → Service → Repository，严禁在 Controller 中直接查库

### 小型需求：仅输出工程计划

如果不满足上述"大型需求"条件，按原有格式仅输出工程实施计划，不追加 PRD 内容块。
