---
name: sync-docs
description: 扫描项目改动并自动同步更新 .docs/ 目录下的 9 个文档。
...
2. **定位文档**：
    - 若修改了 `src/components` -> 更新 `agent-component-map.md`
    - 若修改了 `server/routes` -> 更新 `agent-api-reference.md`
    - 若修改了业务逻辑 -> 更新 `business-overview.md` 和 `agent-file-map.md`
3. **静默更新**：直接修改对应的 Markdown 文件，确保“改 X 找哪里”的索引依然准确。
4. **汇报**：简洁告知用户：“已根据本次改动同步更新了 3 个文档文件”。