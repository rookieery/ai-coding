...
- **按文件类型定制的拆分标准 (SRP)**：
  - UI 组件文件 (.tsx/.vue)：超过 **400 行**或 JSX/Template 嵌套超过 4 层时触发警告，必须考虑抽离。
...
- **异常捕获与全局错误处理**：
  ...
  - 业务错误必须抛出自定义异常（如 `BusinessException`），交由全局异常处理器统一拦截并格式化返回。
- **并发与性能意识**：
  - 在处理可能存在并发的业务时，必须考虑并使用乐观锁、悲观锁或分布式锁。
  - 数据库查询必须考虑索引，严禁在循环中执行 SQL 查询（N+1 问题）。
...
- **唤起指令**：你必须主动通过运行 `refactor-expert` 子智能体来接管该文件的模块化拆分任务。
  - 重构特定文件示例：`请用 refactor-expert 重构 gomoku-web/src/games/gomoku/gameLogic.ts`
...
4. **安全健壮**：所有的新增入参是否都做了边界校验？数据库操作是否考虑了并发或事务？
5. **文档一致性**：是否已经调用 `/sync-docs` 技能更新了 `.docs/` 目录下的速查索引与技术文档？
6. **任务留痕**：是否有未完成的子任务、遇到的问题或改进建议需要记录到 `.docs/todo.md`？
